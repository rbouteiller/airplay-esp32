...
#define ES8311_REG09_SDPIN      0x09  // Serial data port in  (I2S format + resolution)
...
#define ES8311_REG32_DAC_VOL    0x32  // DAC volume: 0x00=-95.5dB, 0xBF=0dB, 0xFF=+32dB
...
static const es8311_coeff_t s_coeff_div[] = {
  ...
  { 5644800, 44100, 0x01, 0x00, 0x01, 0x01, 0x00, 0x00, 0xFF, 0x04, 0x10, 0x10},
  { 6144000, 48000, 0x01, 0x00, 0x01, 0x01, 0x00, 0x00, 0xFF, 0x04, 0x10, 0x10},
  // 384× for higher quality
  {16934400, 44100, 0x01, 0x00, 0x01, 0x01, 0x00, 0x00, 0xFF, 0x06, 0x10, 0x10},
  {18432000, 48000, 0x01, 0x00, 0x01, 0x01, 0x00, 0x00, 0xFF, 0x06, 0x10, 0x10},
};
...
static esp_err_t es8311_detect(i2c_master_bus_handle_t bus) {
  ...
  ESP_LOGW(TAG, "No ES8311 detected at 0x%02X or 0x%02X", ES8311_ADDR, ES8311_ADDR | 1);
...
static esp_err_t es8311_configure_clocks(uint32_t sample_rate) {
  ...
  err = es8311_reg_write(ES8311_REG02_CLK2,
...
static esp_err_t es8311_init(void *i2c_bus) {
  ...
  err = board_i2c_add_device(bus, s_es8311_addr, I2C_LINE_SPEED, &s_es8311_device);
  if (err != ESP_OK) {
...
static void es8311_set_volume(float volume_airplay_db) {
  ...
  if (volume_airplay_db > 0.0f)  volume_airplay_db = 0.0f;
  if (volume_airplay_db < -30.0f) volume_airplay_db = -30.0f;
