...
int32_t airplay_get_volume_q15(void) { return g_volume_q15; }
...
static size_t build_setup_bplist(uint8_t *out, size_t capacity,
                                 ...
                                 uint16_t control_port, uint16_t timing_port,
                                 uint16_t event_port) {
  if (capacity < 160) {
    return 0;
  }
...
static int read_encrypted_block(int socket, uint8_t *buffer,
                                size_t buffer_size) {
  ...
  if (block_len == 0 || block_len > ENCRYPTED_BLOCK_MAX ||
  ...
  if (crypto_aead_chacha20poly1305_ietf_decrypt(
          ...
          sizeof(len_buf), nonce, current_session->decrypt_key) != 0) {
    free(encrypted);
...
// Format: [2-byte length (little-endian)][encrypted data + 16-byte tag]
static int write_encrypted_frame(int socket, const uint8_t *data,
                                 size_t data_len) {
  if (!current_session || !encrypted_mode) {
    // Expected during session teardown - not an error
    return -1;
  ...
  while (offset < data_len) {
    ...
    crypto_aead_chacha20poly1305_ietf_encrypt(
        ...
        NULL, nonce, current_session->encrypt_key);
...
static int send_response(int client_socket, int status_code,
...
static int send_rtsp_response(int client_socket, int status_code,
...
static void handle_info_request(int client_socket, const char *request) {
  ...
  plist_dict_string(&p, "model",
                    "AudioAccessory5,1"); // HomePod mini model = speaker
  ...
  // type 96 = dynamic RTP payload, audioInputFormats/audioOutputFormats are
  // bitmasks
  ...
  send_response(client_socket, 200, "OK", "text/x-apple-plist+xml", body,
                body_len);
}
...
static const uint8_t *get_body(const char *request, size_t request_len,
                               size_t *body_len) {
...
static void handle_post_request(int client_socket, const char *request,
                                size_t request_len, const char *path) {
  ...
  if (strstr(path, "/pair-setup")) {
    ...
    if (!current_session) {
      ...
      if (!current_session) {
        ...
        send_rtsp_response(client_socket, 500, "Internal Error", cseq, NULL,
                           NULL, 0);
        return;
      }
    }
    ...
    if (!response) {
      ESP_LOGE(TAG, "Failed to allocate response buffer");
      send_rtsp_response(client_socket, 500, "Internal Error", cseq, NULL, NULL,
                         0);
      return;
    }
    ...
    if (body && body_len > 0) {
      ...
      if (state && state_len == 1) {
        switch (state[0]) {
        case 1:
          err = hap_pair_setup_m1(current_session, body, body_len, response,
                                  2048, &response_len);
          break;
        case 3:
          err = hap_pair_setup_m3(current_session, body, body_len, response,
                                  2048, &response_len);
          break;
        case 5:
          err = hap_pair_setup_m5(current_session, body, body_len, response,
                                  2048, &response_len);
          break;
        default:
          break;
    ...
    if (err == ESP_OK && response_len > 0) {
      ...
      ESP_LOGE(TAG, "Pair-setup failed: err=%d, response_len=%zu", err,
               response_len);
      // Return error TLV
    ...
    if (body && body_len > 0) {
      ...
      if (state && state_len == 1) {
        if (state[0] == 0x01) {
          err = hap_pair_verify_m1(current_session, body, body_len, response,
                                   1024, &response_len);
        } else if (state[0] == 0x03) {
          err = hap_pair_verify_m3(current_session, body, body_len, response,
                                   1024, &response_len);
        } else {
        ...
        if (current_session->pair_verify_state == 0) {
          ...
          ESP_LOGE(TAG, "Unexpected state for raw format: %d",
                   current_session->pair_verify_state);
    ...
    if (err == ESP_OK && response_len > 0) {
      ...
      ESP_LOGE(TAG, "Pair-verify failed, err=%d, response_len=%zu", err,
               response_len);
      // Return error
      send_rtsp_response(client_socket, 200, "OK", cseq,
                         "Content-Type: application/octet-stream\r\n",
                         "\x06\x01\x04\x07\x01\x02",
                         6); // State=4, Error=2 (auth)
    }
...
static void parse_sdp(const char *sdp, size_t len) {
  ...
  // Format: a=fmtp:96 <frame_length> <compatible_version> <bit_depth> <pb> <mb>
  // <kb>
  //                   <num_channels> <max_run> <max_frame_bytes> <avg_bit_rate>
  //                   <sample_rate>
  ...
  if (rsaaeskey && aesiv) {
    ...
    ESP_LOGW(
        TAG,
        "RAOP RSA key decryption not implemented - audio may be encrypted");
  }
...
static void handle_announce_request(int client_socket, const char *request,
                                    size_t request_len) {
  int cseq = parse_cseq(request);
...
static void handle_setup_request(int client_socket, const char *request,
                                 size_t request_len) {
  int cseq = parse_cseq(request);
  ...
  if (body && body_len > 0) {
      ...
      if (bplist_get_streams_count(body, body_len, &stream_count)) {
        for (size_t i = 0; i < stream_count; i++) {
          ...
          if (bplist_get_stream_info(body, body_len, i, &stream_type,
            ...
            if (bplist_get_stream_kv_info(body, body_len, i, kv, 16,
                                          &kv_count)) {
    ...
    uint8_t
        ekey_encrypted[64]; // Encrypted key (typically 16 or 32 bytes + 16 tag)
    size_t ekey_len = 0;
    uint8_t eiv[16];
    size_t eiv_len = 0;
    ...
    bool has_stream_crypto = bplist_find_stream_crypto(
        body, body_len, 96, ekey_encrypted, sizeof(ekey_encrypted), &ekey_len,
        eiv, sizeof(eiv), &eiv_len, shk, sizeof(shk), &shk_len);
    ...
    if (!has_stream_crypto || (!has_ekey && !has_shk)) {
      has_ekey = bplist_find_data_deep(body, body_len, "ekey", ekey_encrypted,
                                       sizeof(ekey_encrypted), &ekey_len);
      has_eiv = bplist_find_data_deep(body, body_len, "eiv", eiv, sizeof(eiv),
                                      &eiv_len);
      has_shk = bplist_find_data_deep(body, body_len, "shk", shk, sizeof(shk),
                                      &shk_len);
    }
    ...
    if (has_shk && shk_len >= 16) {
      ...
      encryption_set = true;
    } else if (has_ekey && ekey_len > 16 && current_session &&
               current_session->session_established) {
      uint8_t nonce[12] = {0};
      ...
      int ret = crypto_aead_chacha20poly1305_ietf_decrypt(
          decrypted_key, &decrypted_len, NULL, ekey_encrypted, ekey_len, NULL,
          0, nonce, current_session->shared_secret);
      ...
      if (ret == 0 && decrypted_len >= 16) {
        ...
        ret = crypto_aead_chacha20poly1305_ietf_decrypt(
            decrypted_key, &decrypted_len, NULL, ekey_encrypted, ekey_len, NULL,
            0, nonce, current_session->encrypt_key);
    ...
    if (!encryption_set && current_session &&
        current_session->session_established) {
      audio_encrypt_t audio_encrypt = {0};
      audio_encrypt.type = AUDIO_ENCRYPT_CHACHA20_POLY1305;

      esp_err_t err = hap_derive_audio_key(current_session, audio_encrypt.key,
  ...
  ESP_LOGI(TAG, "SETUP: stream_type=%lld", (long long)stream_type);
  ...
  if (stream_type == 103) {
    ...
    // We just need the port numbers to tell the client - audio_receiver will
    // bind later
    int temp_socket;
    ...
    if (audio_stream.data_port == 0) {
      ...
      if (temp_socket > 0)
        close(temp_socket); // Close after getting port
    }
    if (audio_stream.control_port == 0) {
      temp_socket = create_udp_socket(&audio_stream.control_port);
      if (temp_socket > 0)
  ...
  if (is_bplist) {
    ...
    size_t plist_len =
        build_setup_bplist(plist_body, sizeof(plist_body), stream_type,
                           response_data_port, audio_stream.control_port,
                           audio_stream.timing_port, audio_stream.event_port);
    if (plist_len == 0) {
    ...
    snprintf(transport_response, sizeof(transport_response),
             ...
             "Session: 1\r\n",
             audio_stream.data_port, audio_stream.control_port,
...
static void handle_record_request(int client_socket, const char *request) {
  ...
  if (audio_stream.stream_type == 103) {
    ...
    esp_err_t err =
        audio_receiver_start(audio_stream.data_port, audio_stream.control_port);
    if (err != ESP_OK) {
  ...
  int sample_rate =
      audio_stream.sample_rate > 0 ? audio_stream.sample_rate : 44100;
...
static void handle_set_parameter_request(int client_socket, const char *request,
                                         size_t request_len) {
  int cseq = parse_cseq(request);
  size_t body_len;
  const uint8_t *body = get_body(request, request_len, &body_len);
  ...
  if (content_type && strstr(content_type, "text/parameters")) {
    ...
    if (body) {
      ...
      if (strstr((const char *)body, "progress:")) {
             ...
             strstr(content_type, "application/x-apple-binary-plist")) {
    // Binary plist - might contain timing info
    if (body && body_len >= 8 && memcmp(body, "bplist00", 8) == 0) {
      int64_t value;
      if (bplist_find_int(body, body_len, "networkTimeSecs", &value)) {
      ...
      if (bplist_find_real(body, body_len, "rate", &rate)) {
        ESP_LOGI(TAG, "SET_PARAMETER has rate=%.2f", rate);
             ...
             strstr(content_type, "application/x-dmap-tagged")) {
  } else if (body && body_len > 0) {
    // Log unknown content type
    char ct_buf[64] = "unknown";
    if (content_type) {
      sscanf(content_type, "Content-Type: %63s", ct_buf);
    }
    // Check if it's a bplist anyway
    if (body_len >= 8 && memcmp(body, "bplist00", 8) == 0) {
      int64_t value;
      if (bplist_find_int(body, body_len, "networkTimeSecs", &value)) {
        ESP_LOGI(TAG, "SET_PARAMETER(%s) has networkTimeSecs=%lld", ct_buf,
                 (long long)value);
      }
    }
  }
...
static void process_rtsp_buffer(int client_socket, uint8_t *buffer,
  ...
  while (*buf_len > 0) {
    ...
    if (total_len > RTSP_BUFFER_LARGE) {
      ESP_LOGE(TAG, "RTSP message too large: %zu bytes (max %d)", total_len,
               RTSP_BUFFER_LARGE);
      free(header_str);
      *buf_len = 0;
      break;
    ...
    char method[32] = {0};
    char path[256] = {0};
    int parsed = sscanf(header_str, "%31s %255s", method, path);

    // Log all methods for debugging
    ESP_LOGI(TAG, "RTSP: %s %s", method, path);

    if (parsed < 1 || method[0] == '\0') {
      ESP_LOGW(TAG, "Failed to parse method (parsed=%d, recv_len=%zu)", parsed,
               total_len);
      ...
      uint64_t rtp_time = 0;
      if (body && body_len > 0) {
        ESP_LOGI(TAG, "SETRATEANCHORTI body: %zu bytes, bplist=%d", body_len,
                 (body_len >= 8 && memcmp(body, "bplist00", 8) == 0));
        ...
        ESP_LOGI(TAG, "SETRATEANCHORTI parsed: secs=%llu, rtp=%llu, rate=%.1f",
        ...
        if (network_time_secs != 0 && rtp_time != 0) {
          ...
          ESP_LOGW(TAG, "SETRATEANCHORTI missing timing data");
        }
      } else {
        ESP_LOGW(TAG, "SETRATEANCHORTI no body");
      }
      ...
      send_rtsp_response(client_socket, 200, "OK", cseq, NULL, NULL, 0);
    } else if (strcmp(method, "SETPEERS") == 0 ||
               strcmp(method, "SETPEERSX") == 0) {
      int cseq = parse_cseq(request);
      size_t body_len;
      const uint8_t *body = get_body(request, total_len, &body_len);
      ESP_LOGI(TAG, "%s: body_len=%zu", method, body_len);
      // Parse peer list (binary plist with "Addresses" array of strings)
      if (body && body_len >= 8 && memcmp(body, "bplist00", 8) == 0) {
        // Try to find peer addresses
        // For now just log that we got a plist
        ESP_LOGI(TAG, "SETPEERS: got bplist");
      } else if (body && body_len > 0) {
        // Log first few bytes to understand format
        ESP_LOGI(TAG, "SETPEERS data: %02x %02x %02x %02x...", body[0],
                 body_len > 1 ? body[1] : 0, body_len > 2 ? body[2] : 0,
                 body_len > 3 ? body[3] : 0);
      }
      send_rtsp_response(client_socket, 200, "OK", cseq, NULL, NULL, 0);
    } else {
    ...
    if (encrypted_mode) {
      while (server_running && encrypted_mode) {
        if (buf_len >= buf_capacity - 1024) {
          ...
          if (!new_buf) {
            ESP_LOGE(TAG, "Failed to grow RTSP buffer to %zu bytes",
                     new_capacity);
            goto cleanup;
          }
          buffer = new_buf;
        ...
        int block_len = read_encrypted_block(client_socket, buffer + buf_len,
