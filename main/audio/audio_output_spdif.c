...
#define SPDIF_BUF_BYTES (SPDIF_BLOCK * (BMC_BITS / 8) * I2S_CHANNELS \
                         / SPDIF_BUF_DIV)
  ...
  while (p < (const uint8_t *)src + size) {
    ...
    *(spdif_ptr + 1) =
    ...
    if (spdif_ptr >= &spdif_buf[SPDIF_BUF_WORDS]) {
      ...
      i2s_channel_write(tx_handle, spdif_buf, sizeof(spdif_buf),
...
esp_err_t audio_output_init(void) {
  ...
  ESP_LOGI(TAG, "SPDIF output ready  rate=%d×%d  dma=%d×%d",
           SAMPLE_RATE, BMC_FACTOR, DMA_BUF_FRAMES, DMA_BUF_COUNT);
  return ESP_OK;
}
