...
// Fixed output attenuation to prevent clipping (applied in addition to AirPlay
// volume)
...
static void apply_output_attenuation(int16_t *buffer, size_t samples) {
  ...
  int32_t combined_scale =
      ((int32_t)OUTPUT_ATTENUATION_SCALE_Q15 * airplay_vol) >> 15;
...
static void audio_playback_task(void *pvParameters) {
  ...
  const size_t SAMPLE_CAPACITY = SAMPLES_PER_READ + 1;
  int16_t *pcm_buffer = malloc(SAMPLE_CAPACITY * 2 * sizeof(int16_t));
  if (!pcm_buffer) {
  ...
  while (1) {
    ...
    size_t samples = audio_receiver_read(pcm_buffer, SAMPLE_CAPACITY);

    if (samples > 0) {
      apply_output_attenuation(pcm_buffer, samples * 2);
      size_t bytes = samples * 2 * sizeof(int16_t);
      i2s_channel_write(i2s_tx_handle, pcm_buffer, bytes, &bytes_written,
                        portMAX_DELAY);
      silence_count = 0;
      taskYIELD();
    } else {
      size_t bytes = SAMPLES_PER_READ * 2 * sizeof(int16_t);
      i2s_channel_write(i2s_tx_handle, silence, bytes, &bytes_written,
                        pdMS_TO_TICKS(10));
      silence_count++;
      vTaskDelay(1);
    }
...
void app_main(void) {
  ...
  xTaskCreatePinnedToCore(audio_playback_task, "audio_play", 4096, NULL, 7,
                          NULL, AUDIO_TASK_CORE);
